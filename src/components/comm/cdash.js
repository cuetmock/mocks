...
const Sdash = () => {
  const [firstName, setFirstName] = useState(''); // Default value for demo purposes
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost/my-app/src/api/dashboard-data.php', { withCredentials: true });

        if (response.data.error) {
          setError(response.data.error);
        } else {
          const { first_name, email, avgScore } = response.data;
          setFirstName(first_name || ''); // Fallback to empty string if first_name is undefined
          setEmail(email || ''); // Fallback to empty string if email is undefined
          setAvgScore(avgScore || 0); // Fallback to 0 if avgScore is undefined
        }
      } catch (error) {
    ...
    <div className="container mx-auto p-4 bg-gray-100 min-h-screen">
        ...
        <div className="relative profile-dropdown mx-3">
          ...
          {isDropdownVisible && (
            <div className="dropdown-menu absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2">
              <a href="/uprofile" className="block px-4 py-2 text-gray-800 hover:bg-gray-200">Update Profile</a>
      ...
      <nav className="bg-black p-4 rounded-lg shadow mb-4">
        <ul className="flex space-x-4 justify-around">
          <li><a href="/sdash" className="text-white font-bold navbar-item py-2 px-4 rounded-lg">Dashboard</a></li>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="col-span-2 space-y-4">
          <div className="bg-blue-100 p-4 rounded-lg text-center relative">
            ...
            <a href="/mock" className="mt-4 bg-blue-500 text-white p-2 rounded">Start Scoring</a>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <a href="/mock" className="text-blue-600 font-semibold mb-2">CUET Mock Papers</a>
            ...
            <a href="/mock" className="text-blue-600 font-semibold mb-2 mt-2">English Mock Papers</a> <br />
            <a href="/mock" className="text-blue-600 font-semibold mb-2">General Test Mock Papers</a>
